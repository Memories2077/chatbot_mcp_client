...
| LLM routing logic in mcp-gen | ❌ **Missing** | `src/utils/genai.ts` still calls Gemini/Groq directly |
...
| Memory Persistence | 📋 Planned | Enable `memory.enabled=true` in MetaClaw config |
